...
function setupCanvasTouch() {
  ...
  cv.addEventListener('touchmove', e => {
    ...
    if (mode === 'drag' && e.touches.length === 1) {
      const t  = e.touches[0];
      const dx = (t.clientX - lastX) / rect.width  * 100;
      const dy = (t.clientY - lastY) / rect.height * 100;
      ...
      const scale = Math.min(200, Math.max(10, Math.round(pinchStartScale * d / pinchStartDist)));
      cfg.charScale = scale;
  ...
  cv.addEventListener('touchcancel', () => { mode = null; });
}
